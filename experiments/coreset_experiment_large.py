...
dragon_ply_file = join("Data", "dragon_vrip.ply")

# Read the point cloud and draw the bunny
def get_dragon_mesh():
    mesh = o3d.io.read_triangle_mesh(dragon_ply_file)
    mesh.compute_vertex_normals()
    return mesh

mesh_dragon = get_dragon_mesh()
S = np.asarray(mesh_dragon.vertices)
...
filepath = os.path.join(os.getcwd(), "output_data", "coreset_MEB_large.pickle")
...
results["exact"]["optimal_radius"] = 0.011375111172962258

output_filepath = os.path.join(
    output_directory_images,
    "coreset_MEB_primal_iteration_large.pdf",
)
...
output_filepath = os.path.join(
    output_directory_images,
    "coreset_MEB_radius_iteration_large.pdf",
)
...
output_filepath = os.path.join(
    output_directory_images,
    "coreset_MEB_cardinality_large.pdf",
)
...
output_filepath = os.path.join(
    output_directory_images,
    "coreset_MEB_primal_time_large.pdf",
)
